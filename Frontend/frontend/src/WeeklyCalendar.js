...
export const WeeklyCalendar = (props) => {
    ...
    return (
        <div className='weekly-calendar'>
            ...
            {isDetail !== - 1 ? <EventDetail reload={() => reload()} nbr={isDetail} event={eventList[isDetail]} closeDetail={() => closePopup()} /> : isAdd ? <AddPopup user={props.user} ajouterEvent={(x) => props.ajouterEvent(x)} calendarList={() => props.calendarList} setisAdd={() => setisAdd(false)} /> : null}
        </div>
    )
}
