...
export const WeeklyCalendar = (props) => {
    ...
    return (
        <div className='weekly-calendar'>
            ...
            {isDetail !== - 1 ? <EventDetail nbr={isDetail} event={eventList[isDetail]} closeDetail={() => closePopup()}/> : isAdd ? <AddPopup eventList={props.fullEventList} ajouterEvent={(x) => props.ajouterEvent(x)} calendarList={() => props.calendarList} setisAdd={() => setisAdd(false)}/> : null}
        </div>
    )
}
